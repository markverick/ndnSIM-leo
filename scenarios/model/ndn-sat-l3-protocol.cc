...
#include "ndn-sat-net-device-transport.h"
...
NS_LOG_COMPONENT_DEFINE("ndn.SatL3Protocol");

namespace ns3 {
namespace ndn {

const uint16_t SatL3Protocol::ETHERNET_FRAME_TYPE = 0x7777;
const uint16_t SatL3Protocol::IP_STACK_PORT = 9695;

NS_OBJECT_ENSURE_REGISTERED(SatL3Protocol);

TypeId
SatL3Protocol::GetTypeId(void)
{
  static TypeId tid =
    TypeId("ns3::ndn::SatL3Protocol")
      .SetGroupName("ndn")
      .SetParent<Object>()
      .AddConstructor<SatL3Protocol>()

      .AddTraceSource("OutInterests", "OutInterests",
                      MakeTraceSourceAccessor(&SatL3Protocol::m_outInterests),
                      "ns3::ndn::SatL3Protocol::InterestTraceCallback")
      .AddTraceSource("InInterests", "InInterests",
                      MakeTraceSourceAccessor(&SatL3Protocol::m_inInterests),
                      "ns3::ndn::SatL3Protocol::InterestTraceCallback")

      ////////////////////////////////////////////////////////////////////

      .AddTraceSource("OutData", "OutData", MakeTraceSourceAccessor(&SatL3Protocol::m_outData),
                      "ns3::ndn::SatL3Protocol::DataTraceCallback")
      .AddTraceSource("InData", "InData", MakeTraceSourceAccessor(&SatL3Protocol::m_inData),
                      "ns3::ndn::SatL3Protocol::DataTraceCallback")

      ////////////////////////////////////////////////////////////////////

      .AddTraceSource("OutNack", "OutNack", MakeTraceSourceAccessor(&SatL3Protocol::m_outNack),
                      "ns3::ndn::SatL3Protocol::NackTraceCallback")
      .AddTraceSource("InNack", "InNack", MakeTraceSourceAccessor(&SatL3Protocol::m_inNack),
                      "ns3::ndn::SatL3Protocol::NackTraceCallback")

      ////////////////////////////////////////////////////////////////////

      .AddTraceSource("SatisfiedInterests", "SatisfiedInterests",
                      MakeTraceSourceAccessor(&SatL3Protocol::m_satisfiedInterests),
                      "ns3::ndn::SatL3Protocol::SatisfiedInterestsCallback")
      .AddTraceSource("TimedOutInterests", "TimedOutInterests",
                      MakeTraceSourceAccessor(&SatL3Protocol::m_timedOutInterests),
                      "ns3::ndn::SatL3Protocol::TimedOutInterestsCallback")
    ;
  return tid;
}

class SatL3Protocol::Impl {
private:
  Impl()
  {
...
void
SatL3Protocol::injectInterest(const Interest& interest)
{
  m_impl->m_internalClientFaceForInjects->expressInterest(interest, nullptr, nullptr, nullptr);
}

void
SatL3Protocol::setCsReplacementPolicy(const PolicyCreationCallback& policy)
{
  m_impl->m_policy = policy;
}

void
SatL3Protocol::initializeManagement()
{
  auto& forwarder = m_impl->m_forwarder;
  using namespace nfd;

  std::tie(m_impl->m_internalFace, m_impl->m_internalClientFace) = face::makeInternalFace(LeoStackHelper::getKeyChain());
  m_impl->m_faceTable->addReserved(m_impl->m_internalFace, face::FACEID_INTERNAL_FACE);

  std::tie(m_impl->m_internalFaceForInjects, m_impl->m_internalClientFaceForInjects) = face::makeInternalFace(LeoStackHelper::getKeyChain());
  m_impl->m_faceTable->addReserved(m_impl->m_internalFaceForInjects, face::FACEID_INTERNAL_FACE + 1);

  m_impl->m_dispatcher = make_unique<::ndn::mgmt::Dispatcher>(*m_impl->m_internalClientFace, LeoStackHelper::getKeyChain());
...
void
SatL3Protocol::initializeRibManager()
{
  using namespace nfd;

  std::tie(m_impl->m_internalRibFace, m_impl->m_internalRibClientFace) = face::makeInternalFace(LeoStackHelper::getKeyChain());
 ...
 */
void
SatL3Protocol::NotifyNewAggregate()
{
  if (m_node == nullptr) {
...
nfd::FaceId
SatL3Protocol::addFace(shared_ptr<Face> face)
{
...
shared_ptr<Face>
SatL3Protocol::getFaceById(nfd::FaceId id) const
{
  return m_impl->m_faceTable->get(id)->shared_from_this();
}

shared_ptr<Face>
SatL3Protocol::getFaceByNetDevice(Ptr<NetDevice> netDevice) const
{
  for (auto& i : *m_impl->m_faceTable) {
    auto transport = dynamic_cast<SatNetDeviceTransport*>(i.getTransport());
...
Ptr<SatL3Protocol>
SatL3Protocol::getSatL3Protocol(Ptr<Object> node)
{
  Ptr<SatL3Protocol> retval = node->GetObject<SatL3Protocol>();
  NS_ASSERT_MSG(retval != nullptr, "SatL3Protocol is not aggregated on this object");
  return retval;
}
