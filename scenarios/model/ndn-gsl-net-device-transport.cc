...
NS_LOG_COMPONENT_DEFINE("ndn.GSLNetDeviceTransport");

namespace ns3 {
namespace ndn {

GSLNetDeviceTransport::GSLNetDeviceTransport(Ptr<Node> node,
  ...
  , m_node(node)
{
  ...
  m_node->RegisterProtocolHandler(MakeCallback(&GSLNetDeviceTransport::receiveFromNetDevice, this),
                                  L3Protocol::ETHERNET_FRAME_TYPE, m_netDevice,
                                  true /*promiscuous mode*/);
}

GSLNetDeviceTransport::~GSLNetDeviceTransport()
{
  NS_LOG_FUNCTION_NOARGS();
}

ssize_t
GSLNetDeviceTransport::getSendQueueLength()
{
  PointerValue txQueueAttribute;
  if (m_netDevice->GetAttributeFailSafe("TxQueue", txQueueAttribute)) {
    Ptr<ns3::QueueBase> txQueue = txQueueAttribute.Get<ns3::QueueBase>();
...
void
GSLNetDeviceTransport::doSend(const Block& packet)
{
  ...
  // send the NS3 packet
  if (this->getLinkType() != ::ndn::nfd::LINK_TYPE_AD_HOC) {
    m_netDevice->Send(ns3Packet, m_netDevice->GetBroadcast(),
                      L3Protocol::ETHERNET_FRAME_TYPE);
  } else {
    std::cout << "FOUND GSL: " << this->getLocalUri() << std::endl;
    m_netDevice->Send(ns3Packet, m_netDevice->GetBroadcast(),
                      L3Protocol::ETHERNET_FRAME_TYPE);
    m_netDevice->Send(ns3Packet, m_netDevice->GetBroadcast(),
                      L3Protocol::ETHERNET_FRAME_TYPE);
...
// callback
void
GSLNetDeviceTransport::receiveFromNetDevice(Ptr<NetDevice> device,
...
Ptr<NetDevice>
GSLNetDeviceTransport::GetNetDevice() const
{
  return m_netDevice;
}
