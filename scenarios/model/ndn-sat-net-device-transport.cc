...
NS_LOG_COMPONENT_DEFINE("ndn.SatNetDeviceTransport");

namespace ns3 {
namespace ndn {

SatNetDeviceTransport::SatNetDeviceTransport(Ptr<Node> node,
  ...
  , m_node(node)
{
  ...
  m_node->RegisterProtocolHandler(MakeCallback(&SatNetDeviceTransport::receiveFromNetDevice, this),
                                  L3Protocol::ETHERNET_FRAME_TYPE, m_netDevice,
                                  true /*promiscuous mode*/);
}

SatNetDeviceTransport::~SatNetDeviceTransport()
{
  NS_LOG_FUNCTION_NOARGS();
}

ssize_t
SatNetDeviceTransport::getSendQueueLength()
{
  PointerValue txQueueAttribute;
  if (m_netDevice->GetAttributeFailSafe("TxQueue", txQueueAttribute)) {
    Ptr<ns3::QueueBase> txQueue = txQueueAttribute.Get<ns3::QueueBase>();
...
void
SatNetDeviceTransport::doSend(const Block& packet)
{
...
// callback
void
SatNetDeviceTransport::receiveFromNetDevice(Ptr<NetDevice> device,
...
Ptr<NetDevice>
SatNetDeviceTransport::GetNetDevice() const
{
  return m_netDevice;
}
