...
#include "ndn-leo-stack-helper.h"
...
NS_LOG_COMPONENT_DEFINE("ndn.LeoStackHelper");

namespace ns3 {
namespace ndn {

LeoStackHelper::LeoStackHelper()
...
void
LeoStackHelper::setCustomNdnCxxClocks()
{
...
void
LeoStackHelper::SetDefaultRoutes(bool needSet)
{
...
void
LeoStackHelper::SetStackAttributes(const std::string& attr1, const std::string& value1,
...
void
LeoStackHelper::setCsSize(size_t maxSize)
{
  m_maxCsSize = maxSize;
}

void
LeoStackHelper::setPolicy(const std::string& policy)
{
...
void
LeoStackHelper::Install(const NodeContainer& c) const
{
...
void
LeoStackHelper::InstallAll() const
{
...
LeoStackHelper::Install(Ptr<Node> node) const
{
  ...
  Simulator::ScheduleWithContext(node->GetId(), Seconds(0), &LeoStackHelper::doInstall, this, node);
  ProcessWarmupEvents();
}
...
void
LeoStackHelper::AddFaceCreateCallback(TypeId netDeviceType,
                                   LeoStackHelper::FaceCreateCallback callback)
{
  m_netDeviceCallbacks.push_back(std::make_pair(netDeviceType, callback));
}

void
LeoStackHelper::UpdateFaceCreateCallback(TypeId netDeviceType,
...
void
LeoStackHelper::RemoveFaceCreateCallback(TypeId netDeviceType,
                                      FaceCreateCallback callback)
{
                                           ...
                                           Ptr<NetDevice> device) const
{
  ...
  // access the shared channel of the link
...
void
LeoStackHelper::Update(const NodeContainer& c)
{
  for (NodeContainer::Iterator i = c.Begin(); i != c.End(); ++i) {
    Update(*i);
  }
...
void
LeoStackHelper::Update(const std::string& nodeName)
{
  Ptr<Node> node = Names::Find<Node>(nodeName);
  Update(node);
}
...
void
LeoStackHelper::disableStrategyChoiceManager()
{
  m_isStrategyChoiceManagerDisabled = true;
}

void
LeoStackHelper::disableForwarderStatusManager()
{
...
void
LeoStackHelper::ProcessWarmupEvents()
{
