...
void sendInterestWithHint(Ptr<Node> gsNode, string prefix, string hintPrefix) {
  // NS_ASSERT_MSG(gsNode->GetId() < m_satellites.size(), "Ground station ID is invalid");
  // Create a name with sequence
  int seq = 0;
  shared_ptr<ndn::Name> nameWithSequence = make_shared<ndn::Name>(prefix);
  nameWithSequence->appendSequenceNumber(seq);
  // Create an interest with the given name
  ...
  vector<ndn::Name> del;
  del.emplace_back(ndn::Name(hintPrefix));
...
public:
  ...
  void Run() {
    ...
    std::string hintPrefix = prefix + "1500"; //redirect to sat 1500
    m_prefix = prefix2;
    // cout << "PREFIX: " << prefix2 << endl;
    // Consumer
    ns3::Simulator::Schedule(Seconds(5), &sendInterestWithHint, node1, m_prefix, hintPrefix);
