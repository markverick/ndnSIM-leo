...
public:
  ...
  void Run() {
    ...
    consumerHelper.SetAttribute("MaxSeq", StringValue("100"));

    consumerHelper.Install(node1).Start(Seconds(1));

    consumerHelper.SetAttribute("Frequency", StringValue("1"));
    consumerHelper.SetPrefix(m_prefix);
    consumerHelper.SetAttribute("StartSeq", StringValue("0"));
    consumerHelper.SetAttribute("MaxSeq", StringValue("100"));
    consumerHelper.Install(node2).Start(Seconds(101));
    ...
    producerHelper.Install(node3).Start(Seconds(1));
