...
#ifndef NFD_DAEMON_FW_NACK_RETX_STRATEGY_HPP
#define NFD_DAEMON_FW_NACK_RETX_STRATEGY_HPP
 ...
 */
class NackRetxStrategy : public Strategy
{
public:
  explicit
  NackRetxStrategy(Forwarder& forwarder, const Name& name = getStrategyName());

  static const Name&
  getStrategyName();

  void
  sendNackOrForward(const Interest& interest, const FaceEndpoint&,
                       const shared_ptr<pit::Entry>& pitEntry);

  void
  sendNackOrForward(const Interest& interest,
                       const shared_ptr<pit::Entry>& pitEntry);
