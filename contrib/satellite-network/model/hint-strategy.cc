                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    NDN_THROW(std::invalid_argument("HintStrategy does not accept parameters"));
  }
...
{
  static const auto strategyName = Name("/localhost/nfd/strategy/hint").appendVersion(0);
  return strategyName;
}

void HintStrategy::sendNackOrForward(const Interest& interest, const FaceEndpoint& faceEndpoint, const shared_ptr<pit::Entry>& pitEntry) {
  // Don't use a forwarding hint when handling NACK
...
void HintStrategy::sendNackOrForward(const Interest& interest, const shared_ptr<pit::Entry>& pitEntry) {
  const fib::Entry& fibEntry = this->lookupFib(interest);
...
void
HintStrategy::afterReceiveInterest(const Interest& interest, const FaceEndpoint& ingress,
...
void
HintStrategy::afterReceiveNack(const lp::Nack& nack, const FaceEndpoint& ingress,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  if (nack.getReason() == lp::NackReason::NO_ROUTE) {
    // Strip the forwarding hint
    if (!nack.getInterest().getForwardingHint().empty()) {
      Interest interestNoHint = Interest(ndn::Name(nack.getInterest().getName()), time::milliseconds(2000));
      interestNoHint.setForwardingHint({});
      sendNackOrForward(interestNoHint, pitEntry);
    } else {
