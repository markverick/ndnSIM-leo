...
#ifndef POINT_TO_POINT_SAT_CHANNEL_H
#define POINT_TO_POINT_SAT_CHANNEL_H
 ...
 * \see TransmitStart
 */
class PointToPointSatChannel : public Channel 
{
public:
  /**
  ...
  /**
   * \brief Create a PointToPointSatChannel
   *
   * By default, you get a channel that has an "infinitely" fast 
   * transmission speed and zero delay.
   */
  PointToPointSatChannel ();

  /**
   * \brief Attach a given netdevice to this channel
   * \param device pointer to the netdevice to attach to the channel
   */
  void Attach (Ptr<PointToPointSatNetDevice> device);

  /**
   * \brief Transmit a packet over this channel
   * \param p Packet to transmit
   * \param src Source PointToPointSatNetDevice
   * \param txTime Transmit time to apply
   * \returns true if successful (currently always true)
   */
  virtual bool TransmitStart (Ptr<const Packet> p, Ptr<PointToPointSatNetDevice> src, Time txTime);
  ...
  /**
   * \brief Get PointToPointSatNetDevice corresponding to index i on this channel
   * \param i Index number of the device requested
   * \returns Ptr to PointToPointSatNetDevice requested
   */
  Ptr<PointToPointSatNetDevice> GetPointToPointDevice (std::size_t i) const;
...
protected:
   ...
   * \returns Ptr to PointToPointSatNetDevice source for the 
   * specified link
   */
  Ptr<PointToPointSatNetDevice> GetSource (uint32_t i) const;

  /**
   * \brief Get the net-device destination
   * \param i the link requested
   * \returns Ptr to PointToPointSatNetDevice destination for 
   * the specified link
   */
  Ptr<PointToPointSatNetDevice> GetDestination (uint32_t i) const;
...
private:
  ...
  /**
   * \brief Wire model for the PointToPointSatChannel
   */
  class Link
  {
public:
    ...
    Ptr<PointToPointSatNetDevice> m_src;   //!< First NetDevice
    Ptr<PointToPointSatNetDevice> m_dst;   //!< Second NetDevice
  };
...
#endif /* POINT_TO_POINT_SAT_CHANNEL_H */
